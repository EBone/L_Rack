...
struct ModuleBrowser : OpaqueWidget {
	...
	void step() override {
		...
		moduleScroll->box.size.y = min_rack(box.size.y - moduleScroll->box.pos.y, moduleList->box.size.y);
		box.size.y = min_rack(box.size.y, moduleScroll->box.getBottomRight().y);
