...
int AudioIO::getDeviceChannels(int device) {
	...
	if (rtAudio) {
		...
		if (getDeviceInfo(device, &deviceInfo))
			return max_rack((int) deviceInfo.inputChannels, (int) deviceInfo.outputChannels);
	}
	else if (driver == BRIDGE_DRIVER) {
		return max_rack(BRIDGE_OUTPUTS, BRIDGE_INPUTS);
	}
	return 0;
}
	...
	if (rtAudio) {
		...
		if (getDeviceInfo(device, &deviceInfo)) {
			...
			if (offset < (int) deviceInfo.inputChannels)
				deviceDetail += stringf("%d-%d in", offset + 1, min_rack(offset + maxChannels, (int) deviceInfo.inputChannels));
			if (offset < (int) deviceInfo.inputChannels && offset < (int) deviceInfo.outputChannels)
				deviceDetail += ", ";
			if (offset < (int) deviceInfo.outputChannels)
				deviceDetail += stringf("%d-%d out", offset + 1, min_rack(offset + maxChannels, (int) deviceInfo.outputChannels));
			deviceDetail += ")";
			return deviceDetail;
		}
	}
